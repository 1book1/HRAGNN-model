...
def to_sparse(x):
    ...
    if len(indices.shape) == 0:  # if all elements are zeros
        return torch.sparse_coo_tensor(indices=indices, values=[], size=x.size(), dtype=x.dtype, device=x.device)

    indices = indices.t()
    values = x[tuple(indices[i] for i in range(indices.shape[0]))]

    # Creating sparse tensor using torch.sparse_coo_tensor
    return torch.sparse_coo_tensor(indices, values, size=x.size(), dtype=x.dtype, device=x.device)
